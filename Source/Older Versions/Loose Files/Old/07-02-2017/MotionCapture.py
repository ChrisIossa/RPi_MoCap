...
class MotionCapture:
    ...
    def findMarkers(self, image):
        ...
        if len(contourList) > 0:  # check if at least 1 marker was found
            ...
            for contour in contourList:
                ...
                box = np.int0(box)
                cv2.contourList(image, [box], 0, (0, 255, 0), 2)
                x, y, x2, y2 = cv2.boundingRect(contour)
                centerX = int (x+(x2/2))
                centerY = int (y+(y2/2))

                # Draw an identifying label on top of each marker
                self.drawText(image, "A" + str(len(contourList) - i), (centerX, centerY - 15), .50, (0, 255, 255), 1)
    ...
    def startCapture(self):
        ...
        while True:
			...
			# Enable the lightring for the camera
            if keyPress == ord('e'):
                enableLightRing.enable()
			# Disable the lightring for the camera
            if keyPress == ord('d'):
                disableLightRing.disable()

            # Decrease the threshold of whitelight capture
            if keyPress == 84:
                self.thresholdValue = self.thresholdValue - 1
                print("Decreasing threshold to {0}".format(self.thresholdValue))

            # Increase the threshold of whitelight capture
            if keyPress == 82:
                self.thresholdValue = self.thresholdValue + 1
